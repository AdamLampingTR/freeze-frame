  ...
  return (
    <div className="dialog-backdrop" onClick={onCancel}>
      <div className="dialog" onClick={(e) => e.stopPropagation()}>
        <h3>
          Dismiss {candidate.prId != null ? `PR ${candidate.prId}` : candidate.commitId.slice(0, 8)}
        </h3>
        <label>
          Reason
        ...
        <label>
          Note <input value={reasonText} onChange={(e) => setReasonText(e.target.value)} />
        </label>
        <div className="dialog-actions">
          <button onClick={onCancel}>Cancel</button>
          <button onClick={() => onConfirm({ reason, kind, reasonText: reasonText || undefined })}>
            Dismiss
          </button>
        </div>
      </div>
